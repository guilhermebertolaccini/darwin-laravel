...
trait Menu
{
    ...
    protected function mainRoute($menu, $data)
    {
        ...
        $menuData['class'] = $data['li_class'] ?? 'nav-item';

        $linkData = ['class' => 'nav-link'];

        if (isset($data['link_attributes']) && is_array($data['link_attributes'])) {
            $linkData = array_merge($linkData, $data['link_attributes']);
        }
        ...
        // Add the menu item and keep a reference to it
        $menuItem = $menu->add($this->createMenuTitle($data['title'] ?? ''), $menuData)
            ->data([
                'order' => $data['order'] ?? 0,
        ...
        $menuItem->link->attr($linkData);

        // Now append the upgrade message HTML inside the <li>, after the link
        if (!empty($data['upgrade_message'])) {
            // Append raw HTML after the link inside the <li>
            $menuItem->append($data['upgrade_message']);
        }
    }
    ...
    protected function createMenuIcon($cutomeIcon = null, $title = null)
    {
        ...
        if (isset($cutomeIcon)) {
            ...
            // Check if icon is already HTML/SVG (contains < or >)
            if (strpos($cutomeIcon, '<') !== false || strpos($cutomeIcon, '>') !== false) {
                // It's already HTML/SVG, return as is with tooltip wrapper if needed
                $icon = $iconTooltip ? "<span $iconTooltip>$cutomeIcon</span>" : $cutomeIcon;
            } else {
                // It's a class name, wrap in <i> tag
