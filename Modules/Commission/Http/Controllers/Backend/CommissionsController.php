...
// dd($data);
        if (multiVendor() == "0") {
            $data->where(function ($q) {
                $q->where('type', 'doctor_commission');

                if (checkPlugin('pharma') == 'active') {
                    $q->orWhere('type', 'pharma_commission');
                }
            });
        } else {
            if (checkPlugin('pharma') != 'active') {
                $data->where('type', '!=', 'pharma_commission');
            }
        }
    ...
    /**
     * Get available commission types based on plugin status
     *
     * @return Response
     */
    public function getAvailableTypes()
    {
        ...
        // Always include Doctor Commission
        $types[] = [
            'label' => 'Doctor Commission',
            'value' => 'doctor_commission'
        ];

        // Include Pharma Commission only if pharma plugin is active
        if (checkPlugin('pharma') == 'active') {
            $types[] = [
                'label' => 'Pharma Commission',
                'value' => 'pharma_commission'
            ];
        }

        // Include Admin Fees only if multi-vendor is enabled
        if (multiVendor() == "1") {
            array_unshift($types, [
                'label' => 'Admin Fees',
                'value' => 'admin_fees'
        ...
        return response()->json(['data' => $types, 'status' => true]);
    }
