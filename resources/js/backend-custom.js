...
import 'node-snackbar/dist/snackbar.min.css'
  ; (function () {
    ...
    /*------------Popover--------------*/
    const initPopovers = () => {
      if (typeof bootstrap !== typeof undefined && bootstrap.Popover) {
    ...
    window.tooltipInit = () => {
      if (typeof bootstrap !== typeof undefined && bootstrap.Tooltip) {
    ...
    if (offcanvasheader !== null && offcanvasheader !== undefined && typeof bootstrap !== typeof undefined && bootstrap.Offcanvas) {
    ...
    window.laravel = {
      ...
      handleMethod: function (e) {
        ...
        if (link.data('confirm')) {
          if (httpMethod == 'GET') {
            window.laravel.verifyConfirmdata(link).then((res) => {
              if (res.isConfirmed) {
            ...
            window.laravel.verifyConfirm(link).then((res) => {
              if (res.isConfirmed) {
                const formID = 'form-' + link.attr('id')
                form = window.laravel.createForm(link, formID)
                if (link.data('type') == 'ajax') {
